...
struct fusion_set_options{
	int pic;			/* Program Independant Code (0 = off) */
	int endian;			/* 0 = little, 1 = big */
	int gen_ucode;		/* Generate microcode for coprocessor instructions */
...
//	fprintf(stream, _("\nNo options available at the moment\n"));
	fprintf(stream, _("\nFusion-Core Options:\n\
				-le		generate little endian output(default)\n\
				-be		generate big endian output\n\
				-PIC	generate position independent code\n\
				-ncp	disable all coprocessor instructions, use as macros\n\
				"));
}
